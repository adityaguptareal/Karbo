// src/components/shared/CreditCard.tsx
import { MapPin, CheckCircle, Leaf, ShoppingCart } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";

/**
 * Shared credit type used by company marketplace + cards
 * (this is independent from any mockData types)
 */
...
export function CreditCard({
  ...
  // Nicely join location, state, country when available
  const locationLabel = [credit.location, credit.state, credit.country]
    .filter(Boolean)
    .join(", ");
    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden hover:shadow-lg transition-all group">
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <Badge
            variant={credit.verified ? "default" : "secondary"}
        ...
        <div className="flex items-center justify-between pt-4 border-t border-border">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              per credit • {credit.credits} available
            </p>
          </div>
          <Button
            variant="default"
